...
class NutricionHolder(val context: Context, val listanutricion:MutableList<Objetivosutricion>) :
  ...
  class ViewHolder(itemView: View):RecyclerView.ViewHolder(itemView){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NutricionHolder.ViewHolder {
        var itemView = LayoutInflater.from(context).inflate(R.layout.item_objetivos_receta, parent, false)
        return NutricionHolder.ViewHolder(itemView)
    }

    override fun onBindViewHolder(holder: NutricionHolder.ViewHolder, position: Int) {
